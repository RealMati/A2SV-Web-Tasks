...
const desc = () => {
  ...
  return (
    <div className="flex py-8 px-20 gap-16">
      <div className="left w-2/3">
        <div className="mb-5">
          <h1 className="text-[#25324b] font-extrabold text-xl mb-3">
            Description
          </h1>
          <p className="text-[#25324b]">{jobs[idx].description}</p>
        </div>
        <div className="mb-5">
          <h1 className="text-[#25324b] font-extrabold text-xl mb-3">
            Responsibilities
          </h1>
          <ul className="text-[#25324b]">
            {jobs[idx].responsibilities &&
              jobs[idx].responsibilities.map((responsibility) => (
        ...
        <div className="text-[#25324b] mb-5">
          <h1 className="text-[#25324b] font-extrabold text-xl mb-3">
            Ideal Candidate we want
          </h1>
          <ul>
            {jobs[idx].ideal_candidate?.traits &&
              jobs[idx].ideal_candidate.traits.map((trait, i) => (
                <li className="flex gap-2" key={i}>
                  <div className="w-[5px] h-[5px] mt-[10px] bg-[#25324b] rounded-full" />
                  <p>{trait}</p>
                </li>
              ))}
          </ul>
      ...
      <div className="right w-1/3">
        <div className="text-[#25324b] mb-5">
          <h1 className="text-[#25324b] font-extrabold text-xl mb-3">About</h1>
          <div>
        ...
        <div className="text-[#25324b]">
          ...
          <div className="flex gap-3  mb-5">
            {jobs[idx].about?.categories &&
              jobs[idx].about.categories.map((category, idx) => (
                <div
                  className={`bg-[#f8f8fd] text-[${
                    idx % 2 === 0 ? "#ffb836" : "#56cdad"
                  }] rounded-3xl px-3 py-1 min-w-16 text-center`}
                  key={idx}
                >
                  {category}
                </div>
        ...
        <div className="text-[#25324b]">
          ...
          <div className="flex flex-wrap gap-3 text-[#4640de] mb-5">
            {jobs[idx].about?.required_skills &&
              jobs[idx].about.required_skills.map((skill, idx) => (
                <div className="bg-[#f8f8fd] py-1 px-3" key={idx}>{skill}</div>
              ))}
            <div className="bg-[#f8f8fd] py-1 px-3">Social Media Marketing</div>
            <div className="bg-[#f8f8fd] py-1 px-3">English</div>
            <div className="bg-[#f8f8fd] py-1 px-3">Copywriting</div>
          </div>
        </div>
      </div>
    </div>
