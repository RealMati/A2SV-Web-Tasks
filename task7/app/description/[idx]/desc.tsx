import { useGetJobQuery } from "@/app/features/api/apiSlice";
...
const Desc = () => {
  ...
  const { data: jobss, error, isLoading } = useGetJobQuery(id);
  const job = jobss?.data;
  console.log(job);
  ...
  if (error)
    return (
      <div className="flex justify-center items-center">Error loading jobs</div>
    );
    ...
    <div className="flex py-8 px-20 gap-16">
      <div className="left w-2/3">
        <div className="mb-5">
          ...
          <p className="text-[#25324b] text-lg">{job?.description}</p>
        </div>
        <div className="mb-5 text-[#25324b]">
          <h1 className="text-[#25324b] font-extrabold text-xl mb-3">
            Responsibilities
          </h1>
          <p className='text-lg font-["Epilogue"] leading-relaxed text-left font-normal'>
        ...
        <div className="text-[#25324b] mb-5">
          ...
          <p className='font-["Epilogue"] text-lg leading-relaxed text-left'>
          ...
          <div className="flex flex-wrap gap-3 text-[#4640de] mb-5">
            {job?.requiredSkills &&
              job.requiredSkills.map((skill: string, idx: number) => (
                <div className="bg-[#f8f8fd] py-1 px-3" key={idx}>
                  {skill}
                </div>
              ))}
